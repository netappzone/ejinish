...
import { Placeholder2Cards } from '@placeholder';
import { Actions } from '@actions';
import Language from '@language';

export default class CollectionScrollHorizontal extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  renderRow = (data) => {
          ...
          <View style = {styles.productItem}>
            <View style = {styles.productImages}>
              <FastImage
                  ...
                  style={{ alignItems: "center", width: this.productImageNewSize.width - 20, height: this.productImageNewSize.height - 20}}
                  resizeMode={FastImage.resizeMode.contain} />
            </View>
            <View>
              <Text style={[styles.title, { marginTop: 10, fontWeight:'500'}]} numberOfLines = {1}>
                {item.title}
              </Text>
              <Text style={[styles.title, { fontSize: 13}]} numberOfLines = {1}>
                {item.vendor}
              </Text>
    ...
    return (
      <View key={AppHelper.newGuid()}>
        <TouchableOpacity
          activeOpacity={0.8}
          onPress={() => this.toShopifyProductDetail({ handle: item.handle,
                                                      title: item.title,
